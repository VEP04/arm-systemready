...
from test_data_builders import expand_template as base_expand_template
from test_mocks import build_char16_payload
from test_mocks import build_df_output
from test_mocks import build_efi_var_bytes
from test_mocks import build_ethtool_ip_address_output
from test_mocks import build_ethtool_ip_link_line
from test_mocks import build_ethtool_ip_link_show_line
from test_mocks import build_fdisk_output
from test_mocks import build_os_indications_var
from test_mocks import build_run_result_from_outcome
from test_mocks import build_sgdisk_partition_output
from test_mocks import check_output_router
from test_mocks import default_device_path
from test_mocks import noop
from test_mocks import normalize_ethtool_tool_path
from test_mocks import passthrough_router
from test_mocks import route_gateway
from test_mocks import run_router
from test_mocks import scenario_truthy
from test_mocks import which_router
...
def build_mock_from_spec(target: str, spec: Any) -> Any:
    ...
    4) Factory that needs the original callable:
       mocks:
         builtins.open:
           factory: test_mocks.passthrough_router
...
def build_blk_devices_scenario_case(
    ...
    prompt_response = _normalize_prompt_response(scenario.get("prompt", "no"))
    ...
    for disk_index, disk in enumerate(disks, start=1):
        ...
        run_responses[
            f"lsblk -rn -o NAME,TYPE /dev/{disk_name} | grep -c part"
        ] = _completed_process_spec(stdout=f"{reported_partition_count}\n")

        if table == "raw" or reported_partition_count == 0:
            run_responses[f"dd if=/dev/{disk_name} bs=1M count=1 > /dev/null"] = (
        ...
        run_responses[
            f"lsblk -rn -o NAME,TYPE /dev/{disk_name} | awk '/part/ {{print $1}}'"
        ] = _completed_process_spec(stdout=_join_stdout_lines(partition_labels))
        ...
        if table == "mbr":
            ...
            for part_index, partition in enumerate(partitions, start=1):
                ...
                run_responses[
                    f"dd if=/dev/{partition_name} bs=1M count=1 > /dev/null"
    ...
    run_responses["lsblk -e 7 -d | grep disk | awk '{print $1}'"] = _completed_process_spec(
        stdout=_join_stdout_lines(disk_names)
    )
    ...
    return {
        "patch_constants": {
            "main": _run_blk_devices_module_main,
        },
        "mocks": {
            "builtins.input": {"return_value": prompt_response},
